...
Do not use `python -m unittest discover tests -v` or an unfiltered `pytest` command for hosted or headless validation. Collection imports mixed-format modules, including scripts that perform work during import. See [`tests/README.md`](https://github.com/Capslockb/tony-stark-hand-control/blob/main/tests/README.md) for the current execution boundary.
