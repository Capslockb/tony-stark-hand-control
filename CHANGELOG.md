...
- **8 test files had hardcoded `C:/Users/<owner>/...` Windows paths** that made the test suite unloadable on any machine other than the original dev box. The "<owner>" was a specific developer's home-directory username, which is also a privacy concern (the path leaks a real name into any test output). Replaced with `os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'tony_stark_hud_control.py'))` so tests run on any platform. The legacy username is no longer referenced anywhere in the public repo.
